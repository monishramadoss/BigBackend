...
inline int findMemoryTypeIndex(uint32_t memoryTypeBits, VkPhysicalDeviceMemoryProperties const& properties, bool shouldBeDeviceLocal) {
    auto lambdaGetMemoryType = [&](VkMemoryPropertyFlags propertyFlags) -> int {
        for (uint32_t i = 0; i < properties.memoryTypeCount; ++i)
        {
            if ((memoryTypeBits & (1 << i)) && ((properties.memoryTypes[i].propertyFlags & propertyFlags) == propertyFlags))
                return i;
        }
        return -1;
    };
    ...
    if (!shouldBeDeviceLocal) {
        constexpr VkMemoryPropertyFlags optimal = VK_MEMORY_PROPERTY_HOST_CACHED_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT | VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT;
        constexpr VkMemoryPropertyFlags required = VK_MEMORY_PROPERTY_HOST_COHERENT_BIT | VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT;
